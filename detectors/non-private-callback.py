...
if len(sys.argv) == 2:
    if sys.argv[1] == "-h" or sys.argv[1] == "--help":
        print("Usage: detectors/non-private-callback.py [path to project]")
        sys.exit()
    else:
...
with open(TMP_PATH + "/.non-private-callback.tmp", "w") as out_file:
    for path in getFiles(PROJ_PATH):
        ...
        for func in results:
            ...
            if " (callback)" in func_type and "#[private]" not in func["macro"]:
                print("[!] Find callback function without #[private] macro.")
